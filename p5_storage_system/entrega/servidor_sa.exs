...
defmodule ServidorSA do
    ...
    def latidos(pid) do
        send(pid, :latido)#Enviamos a nosotros mismos mensaje
            ...
            {:escribe_generico,{clave,valor,con_hash},cliente} -> 
                        ...
                        if estado.primario == Node.self() do
                            if estado.servicio == true do
                                ...
                                estado=
                                if valor_previo == nil do   #Añadimos nuevo dato
                                    valor=
                                    if con_hash == true do
                                        valor = hash("" <> valor)#HASH
                                        valor
                                    else
                                        valor
                                    end               #merge
                                    nuevos_datos = Map.merge(estado.datos, Map.new([{String.to_atom(clave), valor}]))
                                    estado = %{estado|datos: nuevos_datos}
                                    estado
                                else                        #Actualizamos dato
                                    valor=
                                    if con_hash == true do
                                        valor = hash(valor_previo <> valor)#HASH
                                        valor
                                    else
                                        valor
                                    end               #update
                                    nuevos_datos = Map.update(estado.datos, String.to_atom(clave), valor_previo, fn valor_previo -> valor end)
                                    estado = %{estado|datos: nuevos_datos}
                                    estado
                                end#Mandamos datos a copia
                                send({:servidor_sa,estado.copia},{:backup,estado.datos,Node.self()})
                                timeout=50
                                receive do 
                                    :ok ->  send({:cliente_sa,cliente},{:resultado,valor})#Mandamos resultado a cliente
                                after
                                    timeout ->  true
                                ...
                                estado
                            else#No doy servicio
                                estado
                            end
                        else#No soy primario
                            send({:cliente_sa,cliente},{:resultado,:no_soy_primario_valido})
                            IO.inspect("No Prim")
                            estado
                        end
